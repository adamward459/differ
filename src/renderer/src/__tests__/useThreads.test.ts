...
describe('useThreads', () => {
  it('starts with empty threads', () => {
    const { result } = renderHook(() =>
      useThreads('file.ts', leftLines, rightLines),
    );
  ...
  it('does nothing when activeFile is null', () => {
    const { result } = renderHook(() =>
      useThreads(null, leftLines, rightLines),
    );
  ...
  it('marks threads outdated when line content changes', () => {
    ...
    const newLeft: DiffLine[] = [
      { num: 1, content: 'changed', type: 'removed' },
    ];
  ...
  it('preserves original lineContent when appending to existing thread', () => {
    const initialLeft: DiffLine[] = [
      { num: 1, content: 'original', type: 'removed' },
    ];
    const { result, rerender } = renderHook(
      ({ left }) => useThreads('file.ts', left, rightLines),
    ...
    const newLeft: DiffLine[] = [
      { num: 1, content: 'modified', type: 'removed' },
    ];
