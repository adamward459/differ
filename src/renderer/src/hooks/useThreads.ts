...
function buildThreadMap(threads: CommentThread[], file: string | null, side: DiffSide): ThreadMap {
  const map: ThreadMap = {}
  if (!file) return map
...
export function useThreads(
  ...
  const refreshOutdated = useCallback((file: string, left: DiffLine[], right: DiffLine[]) => {
